...
#  Plot the mean values across 3 different timeframes in history using side-by-side bar charts
#  Age by gender
...
#   Plot stacked histograms to show the median income b/w male and female over time
    ...
    for idx, ax in enumerate(axs):
        ...
        df_inc_gender.plot.hist(bins=30, stacked=True, ax=ax, color=['cornflowerblue','hotpink'])
...
#  Plot box charts to show the housing affordability over time
    ...
    df_house_afford_reverse = df_house_afford[['2014_2018','2010_2014','2006_2010']]
    ax = df_house_afford_reverse.plot.box(vert=False, fontsize='small')
    plt.savefig('images/house_afford.png') 

    df_house_own = pd.DataFrame(data=house_own).T
    df_house_own.columns = file_lst
    df_house_own_reverse = df_house_own[['2014_2018','2010_2014','2006_2010']]
    ax = df_house_own_reverse.plot.box(vert=False, fontsize='small')
